...
// NOTE: if you want to add/delete the entries after net_run(), you must protect these lists with a mutex
...
int net_input_handler(uint16_t type, const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    for (proto = protocols; proto; proto = proto->next)
    {
        if (proto->type == type)
        {
            ...
            // occurs software interrupt
            intr_raise_irq(INTR_IRQ_SOFTIRQ);
            return 0;
        }
    }
    // throw away when given unsupported protocol
