...
struct tcp_pcb
{
    ...
    struct ip_endpoint local;   // local address
    struct ip_endpoint foreign; // foreign address
    struct
    {
        uint32_t nxt;
        uint32_t una;
        ...
        uint32_t wl1;
        uint32_t wl2;
    } snd; // needed information while sending
    uint32_t iss;
    struct
    {
        uint32_t nxt;
        uint16_t wnd;
        uint16_t up;
    } rcv; // needed information while recieving
    uint32_t irs;
    uint16_t mtu;
    uint16_t mss;
...
{
    struct timeval first; // first sent time
    struct timeval last;  // last sent time
...
static struct tcp_pcb *tcp_pcb_select(struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    for (pcb = pcbs; pcb < tailof(pcbs); ++pcb)
    {
        ...
        {
            // if address is local address, return pcb for local
            if (!foreign)
                return pcb;
...
static void tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    if (!pcb || pcb->state == TCP_PCB_STATE_CLOSED)
    {
        ...
        {
            // if the given port is not used, returns RST flag
    ...
    case TCP_PCB_STATE_LISTEN:
        // if RST flag is set(RST request), return immediately
        if (TCP_FLG_ISSET(flags, TCP_FLG_RST))
        {
            return;
        }

        // if ACK flag(connection accepted) is set,
        if (TCP_FLG_ISSET(flags, TCP_FLG_ACK))
        {
            tcp_output_segment(seg->ack, 0, TCP_FLG_RST, 0, NULL, 0, local, foreign);
            return;
    ...
    case TCP_PCB_STATE_SYN_SENT:
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_ACK))
        {
            // if a given sequence number is out of range, return RST
        ...
        // check RST
        // check security and precedence(ignore)
        // check SYN
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN))
        {
            pcb->rcv.nxt = seg->seq + 1;
            ...
            else
            {
                ...
                // queue them for processing after the ESTABLISTED state has been reached.
                return;
            }
        }

        // check if neither of the SYN or RST is set
    ...
    case TCP_PCB_STATE_ESTABLISHED:
        if (pcb->snd.una < seg->ack && seg->ack <= pcb->snd.nxt)
        {
            pcb->snd.una = seg->ack;
            tcp_retransmit_queue_cleanup(pcb);
            // ignore: Users should receive positive acknowledgesments for buffers
...
int tcp_init(void)
{
    ...
    {
        errorf("failed ip_protocol_register.");
        return -1;
    }

    if (net_event_subscribe(event_handler, NULL) == -1)
    {
        errorf("failed net_event_subscribe.");
        return -1;
    }
    if (net_timer_register(interval, tcp_timer) == -1)
    {
        errorf("failed net_timer_register()");
        return -1;
    }
