...
#define ARP_CACHE_TIMEOUT_SECONDS 30
...
struct arp_ether_ip
{
    ...
    uint8_t spa[IP_ADDR_LEN]; // prevent alignment
...
static void arp_dump(const uint8_t *data, size_t len)
{
    ...
    fprintf(stderr, "\thln: 0x%04x\n", ntoh16(message->hdr.hln));
    fprintf(stderr, "\tpln: 0x%04x\n", ntoh16(message->hdr.pln));
...
static struct arp_cache *arp_cache_update(ip_addr_t pa, const uint8_t *ha)
{
    ...
    if (!cache)
    {
        errorf("failed to find arp cache with the protocol address: %s", ip_addr_ntop(pa, addr1, sizeof(addr1)));
        return NULL;
    }

    cache->state = ARP_CACHE_STATE_RESOLVED;
    memcpy(cache->ha, ha, ETHER_ADDR_LEN);
    // TODO: clock_gettime(CLOCK_REALTIME, &cache->timestamp)
    gettimeofday(&cache->timestamp, NULL); // deprecated
...
static struct arp_cache *arp_cache_insert(ip_addr_t pa, const uint8_t *ha)
{
    ...
    if (!cache)
    {
        errorf("failed to find arp cache with the protocol address: %s", pa);
        return NULL;
    }
    cache->pa = pa;
    memcpy(cache->ha, ha, ETHER_ADDR_LEN);
...
static void arp_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    {
        errorf("invalid hardware address in an arp header: address=0x%04x, length=0x%04x", ntoh16(msg->hdr.hrd), ntoh16(msg->hdr.hln));
        return;
    }
    if (!(ntoh16(msg->hdr.pro) == ARP_PRO_IP && msg->hdr.pln == IP_ADDR_LEN))
    {
        errorf("invalid protocol address in an arp header: address=0x%04x, length=0x%04x", ntoh16(msg->hdr.pro), ntoh16(msg->hdr.pln));
        return;
    }
    ...
    {
        // when doesn't use a cache, updates the cache
        if (!merge)
        {
            mutex_lock(&mutex);
...
int arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha)
{
    ...
    if (!cache)
    {
        ...
        debugf("cache isn't found, pa=%s", ip_addr_ntop(pa, addr1, sizeof(addr1)));
        mutex_unlock(&mutex);
...
static void arp_timer_handler(void)
{
    ...
    for (entry = caches; entry < tailof(caches); ++entry)
    {
        if (entry->state != ARP_CACHE_STATE_FREE && entry->state != ARP_CACHE_STATE_STATIC)
        {
            ...
            if (diff.tv_sec >= ARP_CACHE_TIMEOUT_SECONDS)
            {
...
int arp_init(void)
{
    ...
    {
        errorf("failed net_protocol_register()");
        return -1;
    }
