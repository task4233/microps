...
void icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if (len < sizeof(*hdr))
    {
        errorf("icmp data is too short: want: >=%d, but got %d", sizeof(*hdr), len);
        return;
    }
    ...
    {
        errorf("invalid checksum, want: 0, got: %d", cksum16((uint16_t *)data, len, 0));
        return;
    }
    ...
    case ICMP_TYPE_ECHO:
        ...
        break;
    default:
        infof("type %d is not implemented", hdr->type);
        break;
    }
}
